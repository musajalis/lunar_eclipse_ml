...
import numpy as np
import DataArrangeTool, AstronomicalPredictionModel
from sklearn import metrics

import os
from pathlib import Path

data_path = os.path.join(Path(os.getcwd()).parent, 'visualization')
os.chdir(data_path)
...
def visualize_prediction(planet, start=0, end=-1):
    ...
    title = f'movement of {planet}'
    plt.title(title)
    plt.savefig()
    diff = len(x) - start
    plt.savefig(f'movement of {planet} for {diff} days')
    plt.show()


def void():
    astronomical = ['sun', 'moon', 'mercury', 'mars', 'venus', 'jupiter', 'saturn']
    n = 200
    for p in astronomical:
        visualize_epicycle(p, 0, 5000)
        pred = AstronomicalPredictionModel(p)
        pred.set_parameters()
